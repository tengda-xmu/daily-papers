...
export function createReader({dialog, assets, api, onSelection, onDocument, onLayout}) {
  const root = document.createElement('section'); root.className = 'reader-pane'; root.setAttribute('aria-label', '论文原文阅读器');
  root.innerHTML = `<header class="reader-header"><div class="reader-title-row"><strong>原文阅读</strong><span class="reader-name"></span><button type="button" data-read="hide" aria-label="收起原文阅读区">收起</button></div>
  ...
  function renderNotes() {
    ...
    for(const item of items){
      ...
      const jump=document.createElement('button');jump.type='button';jump.textContent=`第 ${item.page} 页 · ${NAMES[item.kind]}`;jump.onclick=()=>go(item.page);
  ...
  async function render(view, generation=epoch) {
    ...
    try {
      ...
      const scale=chosen==='fit'?Math.max(.2,(pages.clientWidth-32)/initial.width):Number(chosen)*96/72;
  ...
  function buildPages(){
    ...
    for(let number=1;number<=pdf.numPages;number++){
      const node=document.createElement('div');node.className='reader-page';node.dataset.page=number;node.setAttribute('aria-label',`PDF 第 ${number} 页`);
      const width=Math.max(250,pages.clientWidth-32);node.style.width=`${width}px`;node.style.height=`${width*1.414}px`;
      const overlay=svg('svg',{viewBox:'0 0 1000 1000',preserveAspectRatio:'none','aria-hidden':'true'});overlay.classList.add('reader-overlay');node.append(overlay);pages.append(node);views.push({number,node,overlay,rendered:false,pending:false});
    }
  ...
  function go(number){current=Math.max(1,Math.min(Number(number)||1,views.length || 1));const v=views[current-1];if(v){pages.scrollTop=v.node.offsetTop-pages.offsetTop;queue.unshift(v);drain();}$('.reader-page-number').value=current;}
  let scrollFrame;
  pages.onscroll=()=>{cancelAnimationFrame(scrollFrame);scrollFrame=requestAnimationFrame(()=>{
    if(!views.length)return;const y=pages.getBoundingClientRect().top+Math.min(pages.clientHeight/3,160);let best=views[0];for(const v of views){if(v.node.getBoundingClientRect().top<=y)best=v;else break;}current=best.number;$('.reader-page-number').value=current;
  ...
  $('.reader-page-number').onchange=e=>go(e.target.value);$('.reader-zoom').onchange=()=>{epoch++;buildPages();};
  async function update(nextPaper,nextDoc){
    if(nextPaper===paper && (nextDoc?.hash || '')===(doc?.hash || ''))return;
    if(dirty)await flush();
    const generation=++epoch;observer?.disconnect();views.forEach(clearView);views=[];queue=[];task?.destroy();task=null;pdf=null;
    paper=nextPaper;doc=nextDoc;items=[];revision=0;undo=[];redo=[];selection=null;documentLoading=false;$('.reader-selection').hidden=true;setTool('select');renderNotes();pages.replaceChildren();current=1;controls();
    $('.reader-name').textContent=doc?.name || '尚未载入全文';$('.reader-name').title=doc?.name || '';
    $('.reader-page-count').textContent=`/ ${doc?.page_count || 0}`;
    ...
    state('正在载入原文…');documentLoading=true;controls();
    try {
      if(doc.kind!=='pdf'){
      ...
      $('.reader-page-count').textContent=`/ ${pdf.numPages}`;$('.reader-page-number').max=pdf.numPages;buildPages();renderNotes();state('选择原文可翻译、提问或标记；画线和笔记自动保存。');
    }catch(error){if(generation===epoch)state('原文加载失败：'+error.message,true);}
  ...
  root.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='prev' || action==='next')return go(current+(action==='prev'?-1:1));
      ...
      if(action==='reload'){if(dirty&&!confirm('本页有未保存的修改。请先备份批注；继续将载入本机已保存的版本。'))return;dirty=false;const p=paper,d=doc;doc=null;return update(p,d);}
      ...
      if(['translate','translate-en','question'].includes(action)){
        ...
        const accepted=await onSelection({...selection,action,documentHash:doc.hash});if(accepted){tab('chat');state(action==='question'?'选段已放入右侧提问框，可补充问题后发送。':'已发送选段翻译，请在右侧查看回答和进度。');}
      }
  ...
  return {update,flush,open:()=>{setVisible(true);tab('pdf');},showPage:number=>{setVisible(true);tab('pdf');go(number);},showChat:()=>tab('chat'),discardChanges:()=>{dirty=false;clearTimeout(saveTimer);},hasUnsaved:()=>dirty};
}
