...
$trigger = New-ScheduledTaskTrigger -Daily -At '20:35'
...
$task = New-ScheduledTask -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Description 'Collect public WeChat article leads (at most 3 queries per day) before the 21:00 paper digest. Native restricted article endpoint is disabled.'
