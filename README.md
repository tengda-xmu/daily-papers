...
在“中英翻译 → 全文翻译（保留 PDF 版式）”中，先载入原 PDF，再点击 **翻译并生成 PDF**。完成后直接下载译文文件，也可从回答下方的 **下载原版式译文 PDF** 再次下载。文件沿用原 PDF 的页数、页面尺寸、分栏、图表和独立公式；正文按原文字区域翻译、适配字号，内嵌公式与上下标保留原样。图片内文字不改写，扫描页会指出页码并要求带文字层的 PDF；译文放不下时明确提示，不截断文字或冒充完成。分批进度保存在本机，中断后以相同模型、方向和要求重试可继续。
