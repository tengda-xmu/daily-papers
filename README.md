...
每日检索词由“管理方向”统一生成。单独使用 Elsevier 适配器时仍可传入自定义查询。
...
无需 Elsevier 密钥的 `CNS 子刊专项` 适配器通过 Crossref 按 ISSN 单独检索九本相关子刊，包括 npj Artificial Intelligence、Microsystems & Nanoengineering、Nature Communications 等。每日任务合并当前启用方向的检索词，默认共九次查询；期刊列表位于 `config/cns-search.json`，单刊失败会保留其他期刊结果。
...
每日公开索引把启用方向合并为最多三次查询，复用现有 Scholar 的 24 小时缓存。结果仅接受 ResearchGate 域名下的论文链接，将索引中的 PDF 地址转换为论文主页，不下载全文。来源状态明确标记“公开索引已接入”；保留索引来源、检索片段和年份精度，不把未知年份标成今日发表，也不把检索片段替代已核验摘要。该渠道不能保证覆盖 ResearchGate 全部内容，亦不表示本地账号已经登录。设 `RESEARCHGATE_PUBLIC_INDEX=0` 可仅使用本地导出。
...
默认使用公开索引采集公众号文章线索：Windows 每天 20:35 轮换最多 3 次公开检索，
按已订阅账号和近 30 天发布日期筛选，把标题、短片段和检索入口发布到 `connector-data`，
21:00 的 GitHub 日报读取；快照缺失或超过 24 小时则由云端尝试公开检索。
线索在顶部“科研线索”页单独展示，不占核心与扩展论文名额。

“科研线索”同时提供学术会议、征稿与专题、研究动态、论文库与数据四类内容，支持关键词、研究方向、来源、时间状态筛选及 20/50/100 条分页。公众号汇总近 90 天已发布归档并去重；不读取未同步的私有订阅。会议目录位于 `config/research-leads.json`，每条保留官方入口、核对日期及已知会期，缺少投稿时间的条目不标记为正在征稿。会议与征稿截止分别计算，精确截止时间支持 AoE 时区。

每日流程运行 `python -m src.research_leads`，读取 NeurIPS 与 ICLR 官方 RSS，将近期短公告存入 `data/research-leads.json`。单个来源失败或暂无新公告时保留仍在时间窗口内的历史条目；页面“来源与更新说明”显示实际采集状态。官方会议目录需按官网核对维护，不从公众号片段推断会期或投稿截止；纯页面构建不触发外网采集。
...
核心与扩展默认各 5 篇，可在研究方向设置中调整。资料获取和精读异步补充，不阻断真实论文推荐；候选不足时按实际数量发布。预印本显示版本标识，首次投稿和版本修订日期分开说明。

核心与扩展均在中文标题下常显原英文题目，字号与正文一致；未译完或原题为中文时不重复显示。展开“全文精读”或“摘要解读”可阅读研究问题、方法路线、创新比较、证据发现、局限边界、方向关联和后续研究建议。解读注明原文链接及实际资料范围，并区分作者结果与分析建议。核心只展示核验过的论文原图，缺图自动补充；扩展阅读不显示图片。
...
云端摘要服务复用 `data/analyses/` 缓存。本机助手使用已登录的 Codex 自动补资料和精读，全文优先，可靠完整摘要可先发布摘要解读并继续补全文；搜索片段不冒充完整摘要。任务、分批阅读及发布状态持久化，电脑关闭时暂停，启动后继续。详见[自动精读与批次](docs/recommendation-editions.md)及[原图采集](docs/paper-figures.md)。

## 多研究方向推荐

首页“本期方向”标签可筛选本期论文，“管理方向”打开方向设置。支持最多 12 个方向：新增、改名、暂停、移除；每个方向可分别参与核心推荐和扩展阅读，设置普通、重点或最高优先级。两组篇数分别可设为 1–20 篇。

匹配规则为：命中任一主题关键词；填写限定词时还需命中任一限定词；排除词优先。关键词支持中英文，每行一个短语。常用英文术语放前面，检索会使用前两个主题词和限定词，其余词参与结果筛选；输入的是文字短语，不是各数据库的高级检索语法。

“保存并应用”通过已配对的本机助手把设置保存到 `config/research-directions.json`，供 GitHub 每日任务使用；本机保存已应用副本。返回首页“手动更新”可立即生成。仅在浏览器切换标签不会修改每日配置。设置更新后，已有推荐保留本期范围，下一轮完成时切换到新范围。

检索覆盖全部启用方向；Web of Science 仍合并为最多 3 次请求。各方向先有入选机会，再按优先级分配剩余名额；同一文献不会同时占用核心和扩展位置。不足时显示实际篇数，不混入其他方向凑数。论文标注推荐归属，分区显示各方向覆盖数。每期数据保存方向配置快照，历史归档不会随当前设置改名或重新分类。
...
- 首页专注文献阅读，顶部状态入口可打开“设置”中的来源与采集详情；重点期刊目录也集中在设置页，点击期刊或来源可筛选当期推荐；
- 微信公众号文章在顶部“科研线索”页单独阅读；历史归档保留当期的来源状态和线索快照；
...
已发送的问题下方可点击 **编辑 → 保存并重新发送**；回答下方可 **重新生成**，失败或停止的回答可 **重试**。这些操作会创建新版本，原问题、原回答及其后续对话均保留，通过消息下方的左右箭头切换，也可从选中的版本继续提问。修改问题沿用原截图、翻译方向与范围，使用当前所选模型；更换论文资料后请新建提问，不能用新资料改写旧版本。导出的对话包含当前选中的版本，旧版本的译文 PDF 仍可下载。

输入框按 **Enter 发送、Shift+Enter 换行**，中文输入法选字时不会误发。生成时向上阅读不会被强制拉到底部，点击 **回到最新回复** 可继续跟随输出。编辑问题不会覆盖下方尚未发送的草稿。

论文助手支持 **左侧原文、右侧对话**，中间分隔条可拖动调宽；手机通过“原文 / 对话”切换。载入 PDF 后可连续预览、跳页、缩放，选中文字即可英译中、中译英、就此提问、高亮、加下划线或删除线；扫描页可画线、手绘和添加笔记。点击回答中的 PDF 页码也会定位到左侧原页。“批注”中可修改笔记、颜色或删除标记，支持撤销、重做与刷新恢复。

阅读器顶部可点击 **收起工具栏 / 展开工具栏**，浏览器会记住选择。收起后保留标题栏和 PDF 正文，划词工具仍可使用；**隐藏阅读区** 则关闭整个左侧阅读面板，可从对话顶部的“原文阅读”重新打开。

PDF 区底部常驻 **− / 缩放比例 / ＋**，支持 25%–300% 和适合宽度，顶部工具栏收起后仍可使用。鼠标放在 PDF 上按 **Ctrl（Mac 为 ⌘）＋滚轮** 可围绕当前位置缩放；阅读区获得焦点后也可用 **Ctrl / ⌘ ＋加减键**，按 **Ctrl / ⌘ ＋ 0** 恢复适合宽度。缩放仅改变 PDF 的显示，右侧对话、网页字号和导出的文件不受影响。

批注采用手动保存：点击 **保存批注** 才按论文与 PDF 版本写入本机；**放弃修改** 恢复最近一次加载或保存的内容。收起工具栏后仍可保存或放弃。切换论文、PDF 或关闭对话时，有未保存修改会提示保存、不保存或取消；刷新或关闭标签页会出现浏览器离开提醒。**导出批注 PDF** 只下载包含当前批注（包括未保存修改）的副本，不保存批注记录或加入文献库，保留原始文件、页数和版式。此处“修改”是修改批注和标记，不直接替换论文正文。网页全文可预览与划词翻译；划词需要文字层，扫描页仍可通过右侧截图翻译。预览、标记和导出不调用模型，点击翻译或发送提问时才调用 Codex。

在“中英翻译”中选择 **全文译文 PDF（原版式）** 或 **中英对照 PDF（原版式）**，载入原 PDF 后开始生成。完成后直接下载文件，对话中的译文结果卡片集中提供阅读和下载；左侧“PDF 版本”可切换原文、译文和中英对照，**下载当前 PDF** 下载当前选中的文件。相同原 PDF、模型、方向和翻译要求已有结果时，底部按钮显示 **查看已生成 PDF**，直接打开文件，不提交翻译任务。修改要求后可生成新译文，结果下的 **重新翻译** 会调用模型创建新版本，旧版保留。

译文文件沿用原 PDF 的页数、页面尺寸、分栏、图表和独立公式，中英对照文件按原文、译文逐页配对；正文按原文字区域翻译、适配字号，内嵌公式与上下标保留原样。图片内文字不改写，扫描页会指出页码并要求带文字层的 PDF；译文放不下时明确提示，不截断文字或冒充完成。分批进度保存在本机，中断后以相同模型、方向和要求重试可继续。

“全文翻译（文本对照）”仍可翻译网页全文或扫描页，导出原文、译文及页码标记的重排 PDF。两种译文均绑定对应回答与资料版本，不混入后续提问；“对话 PDF / 对话文本”用于完整会话。下载现有文件不重复调用模型。
...
导航中的 **手动检索** 打开 `search.html`，独立查询现有 11 类来源，支持关键词、日期和来源选择。结果按需分页获取，不再设置每来源 5 / 10 / 25 条的总量上限；每页可显示 20 / 50 / 100 篇。已载入页面可直接返回查看，翻到末尾再请求各来源的后续游标或页码；跨页继续合并 DOI、arXiv 标识及标题作者相同的论文。

新批次追加到已载入结果末尾，保持已读页面稳定。切换“当前结果重排”会对已载入文献统一排序；继续获取后续文献后从第一页显示新排序。页面显示的是已载入篇数，不冒充全库总数。引用导出包含当前来源筛选下**所有已载入页面**的文献。
...
Crossref/CNS、Scopus、OpenAlex、Semantic Scholar 会将三类排序传给原始检索接口；arXiv、PubMed 支持相关性与日期排序，未提供被引计数。Google Scholar、ResearchGate、微信公众号及当前 Web of Science 接入在已取得的候选内重排，页面“来源检索状态”注明范围，不表示全库排名。缓存区分来源、排序、日期和分页位置；切换检索条件或获取新页面可能产生来源 API 请求。
...
- 同时最多运行 4 个来源，每个来源每批 50 秒超时后终止独立进程；可停止并保留已返回的结果。失败后从未完成的分页位置重试，已结束的来源不再请求。成功页面通常缓存 24 小时；Crossref 游标有效期短，页面缓存采用 4 分钟。重复点击同一批次不会多取一页。旧客户端仍兼容原有单批请求，新版页面通过能力检测避免把旧后端当作支持分页。
- CNS 子刊专项对 `config/cns-search.json` 的 9 本期刊使用合并 ISSN 查询。ResearchGate 查询 SerpApi 公开索引并合并本机导出。微信公众号合并订阅 RSS 与本机历史导出后匹配关键词，后续可逐页查询所有公开索引账号，不受日报订阅白名单限制，仍按所选日期过滤。公开索引不代表公众号全量历史；无已核验原文时提供搜狗微信检索入口。
- 微信公众号手动翻页不采用自动采集的每日 3 次查询上限，仍复用按查询和页码区分的缓存，并遵守验证码、403/429 后的共享冷却。自动采集预算保持有效。arXiv 手动查询失败时显示失败，不用无关的每日 RSS 充当查询结果。
- Elsevier / Scopus 使用 `STANDARD` 视图与原生游标继续检索，按所选排序逐页返回，不再在 3 页后截断。API 的 `date` 参数只能限定年份，程序再按期刊日期过滤；本页被过滤为空但还有游标时，仍可继续翻页。Scopus 收录多个出版社，标准视图作者与摘要可能不完整。来源自身的权限、限频、游标过期或检索窗口限制会显示为未完成，保留已有结果，不能解释为全库没有更多文献。
- 每条结果提供原文链接；有可读取的公开 PDF 时支持下载。受机构订阅限制或自动访问受限时，保留原文/全文入口，不承诺所有论文都能自动下载。PDF 获取复用本机助手的 HTTPS 主机白名单、逐跳重定向校验和 50 MB / 300 页限制。
...
打开顶部 **设置 → 重点期刊目录 → 添加期刊 / 管理期刊**，或打开 [管理页](https://tengda-xmu.github.io/daily-papers/journals.html)。运行本机论文助手后，页面复用已有浏览器配对，无需模型对话。
...
双击根目录的 **启动论文助手.cmd**，优先用 **Microsoft Edge** 打开本机页面并自动连接本机已经登录的 Codex，无需复制配对码；未找到 Edge 时会提示并使用系统默认浏览器，不修改系统默认设置。网页每篇论文新增 **Codex 对话**，支持中文总结、追问、翻译、PDF 与配图解释；Edge 中首次使用在线侧栏仍需从本机页面复制一次配对码，默认记住当前浏览器，后续打开网页或重启助手会自动恢复连接，也可随时取消记住。电脑需要保持运行，使用现有 Codex 账号额度。手机浏览公开推荐、科研线索和历史归档无需配对。详见 [连接器说明](connectors/codex_bridge/README.md)。
