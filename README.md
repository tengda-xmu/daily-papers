...
默认使用公开索引采集公众号文章线索：Windows 每天 05:35 轮换最多 3 次公开检索，
按已订阅账号和近 30 天发布日期筛选，把标题、短片段和检索入口发布到 `connector-data`，
06:00 的 GitHub 日报读取；快照缺失或超过 24 小时则由云端尝试公开检索。
...
自动解读仅依据可取得的摘要，每次运行最多尝试“研究方向”中设置的核心与扩展篇数之和（默认 10 篇），通过中文与字段完整性检查后存入 `data/analyses/`，两个分区均复用缓存。资料变化会重新生成。未配置模型、响应不合格或生成失败时继续采集论文并保留已完成的精读，新论文保存在候选库；因此未授权模型时，不能保证每天新增 10 篇中文精读。
...
工作流每天北京时间 06:00 开始更新，采集、筛选和发布完成后显示新一期；也支持 `workflow_dispatch` 手动触发。GitHub 定时调度可能延迟，尤其在整点，因此另设 06:17、06:37 和 07:17 补漏检查。检查在同一更新队列中读取最新数据：当天 06:00 后已发布有效推荐就跳过，手动更新仍可随时执行。它会生成 `site/`、保存 `data/archive/` 历史数据、部署 GitHub Pages，并发送核心论文摘要。
