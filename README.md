...
- Elsevier / Scopus 手动检索使用 `STANDARD` 标准元数据视图，按相关性排序。API 的 `date` 参数只能限定年份，程序再按 Scopus 期刊日期精确过滤；不足所选数量时继续翻页，最多 3 页（每页 25 条）、一次临时网络错误重试。达到上限、授权不足或请求中断时显示具体原因，不将尚未扫描的结果说成“全库无结果”；修复前由首屏未来期刊日期造成的空缓存自动失效。Scopus 收录多个出版社，结果不限 Elsevier 旗下期刊；标准视图的作者与摘要可能不完整。
