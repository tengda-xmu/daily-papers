...
- CNS 子刊专项对 `config/cns-search.json` 的 9 本期刊使用合并 ISSN 查询。ResearchGate 查询 SerpApi 公开索引并合并本机导出。微信公众号优先匹配已订阅 RSS / 同步文章，缺少结果时查询公开索引，共享既有每日预算和限频冷却；公众号结果可能只是检索线索。arXiv 手动查询失败时显示失败，不用无关的每日 RSS 充当查询结果。
