...
- 本机同步与科研线索、AI 前沿使用同一组每日查询；主题及订阅账号按北京时间轮换，同一运行环境复用缓存，预算在北京时间零点换日。
...
查询完成数、缓存复用数、失败与额度暂缓分别记录；失败或没有新文章也更新检查状态，不重写旧文章的采集时间。
页面显示实际失败原因和最近检查时间。公众号状态随独立栏目更新，不必等待生成新的论文批次；历史批次保留当时的采集状态。
